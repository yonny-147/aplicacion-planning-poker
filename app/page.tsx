...
export default function HomePage() {
  ...
  const createRoomMutation = useMutation({
    ...
    onError: (err: Error) => {
      toast.error(
        err.message || "Error de conexión. Por favor intenta de nuevo.",
      );
    },
  });
  ...
  const joinRoomMutation = useMutation({
    mutationFn: async (code: string) => {
      const response = await fetch(`/api/rooms/${code}/validate`);
      const data = await response.json();
      if (!data.exists) throw new Error("La sala no existe o ha sido cerrada");
      return code;
    },
    onSuccess: (code) => {
    ...
    onError: (err: Error) => {
      toast.error(
        err.message || "Error al validar la sala. Por favor intenta de nuevo.",
      );
    },
  });
  ...
  const createRoom = () => {
    ...
    createRoomMutation.mutate(createUserName.trim());
  };
