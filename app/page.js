...
export default function HomePage() {
  ...
  localStorage.setItem(`planning-poker-participant-${data.roomCode}`, data.participantId)
  ...
  const joinRoom = async () => {
    ...
    try {
      ...
      if (!data.exists) {
        alert("La sala no existe o ha sido cerrada")
        return
      }

      router.push(`/join/${roomCode.toUpperCase()}`)
    } catch (err) {
      alert("Error al validar la sala. Por favor intenta de nuevo.")
    }
  }
