...
export async function POST(request, { params }) {
  ...
  // Verificar que el participante existe y es admin
  const participant = Array.isArray(room.participants)
    ? room.participants.find((p) => p.id === participantId)
    : undefined;
  
  console.log("Verificando permisos:", { participantId, participant })
  ...
  const story = await roomStore.addStory(code, { title, description })
