...
export async function GET(request, { params }) {
  ...
  console.log(`[Stream] Iniciando stream para sala ${code}`);
        ...
        if (initialRoom) {
          console.log(`[Stream] Enviando estado inicial. Participantes: ${initialRoom.participants?.length || 0}`);
        ...
        const handleRoomUpdate = (room) => {
          try {
            if (isClosed) return
            console.log(`[Stream] handleRoomUpdate llamado para sala ${code}`);
            if (room) {
              console.log(`[Stream] Enviando actualización. Participantes: ${room.participants?.length || 0}, Historias: ${room.stories?.length || 0}`);
              controller.enqueue(encoder.encode(`data: ${JSON.stringify(room)}\n\n`))
            }
          } catch (error) {
            if (!isClosed) {
              console.error("[Stream] Error en handleRoomUpdate:", error)
            }
          }
        }

        // Registrar listener local (mismo proceso) como fallback
  console.log(`[Stream] Registrando listener local para sala ${code}`);
        roomStore.addListener(code, handleRoomUpdate)

        // Suscribirse a Pub/Sub de Redis para recibir cambios desde otras instancias
        try {
          subscriber = redis.duplicate()
          await subscriber.subscribe(`${REDIS_PREFIX}room-updates:${code}`)
          console.log(`[Stream] Suscrito a canal Redis ${REDIS_PREFIX}room-updates:${code}`)
          subscriber.on('message', (channel, message) => {
            try {
              ...
              if (!isClosed) {
                console.error('[Stream] Error procesando mensaje Redis:', err)
              }
            }
          })
        } catch (e) {
          console.error('[Stream] No se pudo suscribir a Redis Pub/Sub:', e)
        }
        ...
        heartbeatId = setInterval(() => {
          try {
            ...
            if (!isClosed) {
              console.error('[Stream] Error enviando heartbeat:', err)
            }
            clearInterval(heartbeatId)
            heartbeatId = null
          }
        ...
        cleanup = () => {
          try {
            if (isClosed) return
            isClosed = true
            console.log(`[Stream] Limpiando listener para sala ${code}`);
            ...
            if (subscriber) {
              try { subscriber.unsubscribe(`${REDIS_PREFIX}room-updates:${code}`) } catch {}
  ...
  console.log(`[Stream] Stream configurado completamente para sala ${code}`);
      } catch (error) {
