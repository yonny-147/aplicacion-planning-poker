...
export default function JoinRoomPage() {
  ...
  const handleJoin = async () => {
    ...
    try {
      let participantId = localStorage.getItem(`participantId_${roomCode}`)
      if (!participantId) {
      ...
      localStorage.setItem(`participantId_${roomCode}`, data.participantId)
    ...
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      <Card className="w-full max-w-md">
        ...
        <CardContent className="space-y-4">
          ...
          <Button onClick={() => router.push("/")} variant="outline" className="w-full" disabled={isJoining}>
            Cancelar
          </Button>
        </CardContent>
      </Card>
